		...
		<table width="100%" height="26" border="0" cellpadding="0"
			...
			<tbody>
				<tr>
					...
					<td class="fontg" style="margin-top: 2px"><p>已借的书</p></td>
				</tr>
			</tbody>
		</table>
		...
		<div class="clist">
			<ul>
				...
				<?php if(is_array($books)): $i = 0; $__LIST__ = $books;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$book): $mod = ($i % 2 );++$i;?><li><a href="__URL__/details?borrowid=<?php echo ($book["borrowid"]); ?>">
					...
					<p>价格 : <?php echo ($book["book"]["price"]); ?></p>
					<p>上传者：<a href="__APP__/User?userid=<?php echo ($book["offerid"]); ?>"><?php echo ($book["offeruser"]); ?></a></p> 状态： <?php if($book["status"] == 0): ?>未借 <a
						href="__URL__/deletebook?borrowid=<?php echo ($book["borrowid"]); ?>">预订</a> <?php elseif($book["status"] == 1): ?> <span style="color: red;">已预订</span>
					<p>
						<a href="__URL__/retreat?borrowid=<?php echo ($book["borrowid"]); ?>">退订 </a><a
							href="__URL__/borrowed?borrowid=<?php echo ($book["borrowid"]); ?>"> 确认已借</a>
					</p>

					<?php elseif($book["status"] == 2): ?> 等待对方确认 <?php elseif($book["status"] == 3): ?>
					<p>
						对方确认已借 <a href="__URL__/borrowed?borrowid=<?php echo ($book["borrowid"]); ?>">
							确认已借</a>
					</p>
