...
<body>
	...
	$(function(){
		$("#uploadbook").click(function(){
			window.location="__URL__/searchbook";
		});
	});
</script>
	...
	<form id="mybookform">
		<table width="100%" height="26" border="0" cellpadding="0"
			...
			<tbody>
				<tr>
					...
					<td class="fontg" style="margin-top: 2px"><p>我的借架</p></td>
					<td class="fontg" align="right" style="margin-top: 2px">
							<img id="uploadbook" title="上传" src="__BUTTON__/uploadbtn.png" class="btn"/> 
					</td>
					<td>
					&nbsp;
					</td>
		...
		<div class="clist">
			<ul>
				...
				<?php if(is_array($books)): $i = 0; $__LIST__ = $books;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$book): $mod = ($i % 2 );++$i;?><li><a href="__URL__/details?borrowid=<?php echo ($book["borrowid"]); ?>">
						<img class="imgc" src="<?php echo ($book["book"]["cover"]); ?>">
				</a>
					<p>
						<a href="__URL__/details?borrowid=<?php echo ($book["borrowid"]); ?>"><b><?php echo ($book["book"]["bookname"]); ?></b></a>
					</p>
					<p>作者：<?php echo ($book["book"]["author"]); ?></p>
					<p>价格 : <?php echo ($book["book"]["price"]); ?>元</p>
					<p>上传者：<a href="__APP__/User/getUser?userid=<?php echo ($book["offerid"]); ?>" target="_blank"><?php echo ($book["offeruser"]); ?></a></p> 状态： <?php if($book["status"] == 0): ?>未借 <a
						href="__URL__/deletebook?borrowid=<?php echo ($book["borrowid"]); ?>">删除</a> <?php elseif($book["status"] == 1): ?> <span style="color: red;">已预订

					<p>
							<a href="__URL__/borrowed?borrowid=<?php echo ($book["borrowid"]); ?>">确认借出 </a> <a
								href="__URL__/refuse?borrowid=<?php echo ($book["borrowid"]); ?>"> 拒绝</a>
					</p>
					</span>
					
					<?php elseif($book["status"] == 3): ?> 等待对方确认 <?php elseif($book["status"] == 2): ?> 对方确认已借
					<p>
						<a href="__URL__/borrowed?borrowid=<?php echo ($book["borrowid"]); ?>"> 确认借出</a>
					</p>
					<?php else: ?> 已借<?php endif; ?></li><?php endforeach; endif; else: echo "" ;endif; ?>

			</ul>

		</div>
		<div class="page" style="clear: both; margin-left: 20px;">
